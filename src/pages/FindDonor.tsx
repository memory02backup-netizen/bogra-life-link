...
const FindDonor = () => {
  ...
  return (
    <div className="px-4 py-6 space-y-6 animate-fade-in">
      <PageHeader icon={Search} title="ডোনার খুঁজুন" subtitle="জরুরি রক্তের প্রয়োজনে ফেসবুক গ্রুপে পোস্ট করুন" />

      <button onClick={handleCopyAndOpen} className="w-full btn-primary flex items-center justify-center gap-2 py-4">
        {copied ? <><Check className="w-5 h-5" />কপি হয়েছে! গ্রুপে যাচ্ছে...</> : <><Copy className="w-5 h-5" />গ্রুপে পোস্ট করুন<ExternalLink className="w-4 h-4" /></>}
      </button>
        ...
        <div className="space-y-3">
          {steps.map((step, index) => (
            <div key={index} className="flex items-start gap-3">
              <span className="w-6 h-6 rounded-full bg-primary/10 text-primary text-sm font-bold flex items-center justify-center flex-shrink-0">{index + 1}</span>
      ...
      <div className="card-native">
        ...
        <ul className="space-y-2">
          {duties.map((duty, index) => (<li key={index} className="flex items-start gap-2 text-foreground"><span className="text-primary">•</span>{duty}</li>))}
        </ul>
      </div>
    </div>
  );
